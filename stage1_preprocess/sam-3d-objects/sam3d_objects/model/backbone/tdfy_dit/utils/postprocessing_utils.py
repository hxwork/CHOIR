...
def _fill_holes(
    ...
    projection = utils3d.torch.perspective_from_fov(fov_x=fov, fov_y=fov, near=1, far=3)
    views = []
    ...
    for i in tqdm(
        ...
        view = views[i]
        buffers = utils3d.torch.rasterize_triangles(
            rastctx,
            vertices=verts[None],
            faces=faces,
            width=resolution,
    ...
    for cc in to_remove_cc:
        ...
        if len(cc_new_boundary_edge_indices) > 0:
            ...
            cc_new_boundary_edges_cc_center = [
                verts[edges[cc_new_boundary_edge_indices[edge_cc]]].mean(dim=1).mean(dim=0) for edge_cc in cc_new_boundary_edge_cc
            ]
...
def bake_texture(
    ...
    if mode == "fast":
        ...
        for observation, view, projection in tqdm(
            ...
            with torch.no_grad():
                rast = utils3d.torch.rasterize_triangles(
                    rastctx,
        ...
        def render_pt3d_texture(texture, uv, uv_dr=None):
            ...
            render = F.grid_sample(texture_perm, grid, mode='bilinear', padding_mode='border', align_corners=True)
        ...
        mask = 1 - utils3d.torch.rasterize_triangles(rastctx, vertices=(uvs * 2 - 1)[None], faces=faces, width=texture_size,
                                                     height=texture_size)["mask"][0].detach().cpu().numpy().astype(np.uint8)
        texture = cv2.inpaint(texture, mask, 3, cv2.INPAINT_TELEA)
    else:
...
def to_glb(
    ...
    if not with_mesh_postprocess and not with_texture_baking and use_vertex_color:
        ...
        mesh = trimesh.Trimesh(
            vertices,
            faces,
            visual=(trimesh.visual.TextureVisuals(uv=uvs, material=material) if with_texture_baking else None),
        )
