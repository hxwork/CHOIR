...
class MeshExtractResult:
    ...
    def save_ply(self, filename):
        ...
        if self.vertex_attrs is not None:
            # Assuming colors are in [0, 1] for RGB
            colors = self.vertex_attrs.detach().cpu().numpy()
            if colors.shape[-1] >= 3:
        ...
        structured_verts = np.empty(num_verts, dtype=vertex_dtype)
        structured_verts['x'] = verts[:, 0]
        structured_verts['y'] = verts[:, 1]
        structured_verts['z'] = verts[:, 2]
        if colors is not None and colors.shape[-1] >= 3:
            structured_verts['red'] = colors[:, 0]
            structured_verts['green'] = colors[:, 1]
            structured_verts['blue'] = colors[:, 2]

        # Create structured array for faces
        faces_list = [(face,) for face in faces]
        structured_faces = np.array(faces_list, dtype=[('vertex_indices', 'i4', (3,))])

        # Create PlyData and write to file
        el_verts = PlyElement.describe(structured_verts, 'vertex')
        el_faces = PlyElement.describe(structured_faces, 'face')
