...
from utils3d.numpy.maps import (build_mesh_from_map, depth_map_edge, normal_map_edge, point_map_to_normal_map, uv_map)

np.acos = np.arccos  # sam3d_objects-3dfy version of numpy doesn't have acos

MeshAndTexture = namedtuple("mesh_and_texture", ["faces", "vertices", "vertex_colors", "vertex_uvs"])
...
def mesh_from_pointmap(
    ...
    if filter_edges:
        mask = mask & ~(depth_map_edge(depth, rtol=depth_edge_rtol, mask=mask) & normal_map_edge(normals, tol=depth_edge_tol, mask=normals_mask))

    faces, vertices, vertex_colors, vertex_uvs = build_mesh_from_map(
        pointmap,
        image.astype(np.float32),
        uv_map(width=width, height=height),
        mask=mask,
        tri=True,
    )
