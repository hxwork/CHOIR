...
from pytorch3d.renderer import (BlendParams, MeshRasterizer, MeshRenderer, PerspectiveCameras, RasterizationSettings, SoftSilhouetteShader, TexturesVertex)
from pytorch3d.structures import Meshes
from pytorch3d.transforms import Transform3d, quaternion_to_matrix
from scipy.ndimage import (binary_dilation, binary_erosion, binary_fill_holes, label, minimum_filter)
...
def check_occlusion(mask, point_map, min_region_area=25, border_thickness=5, z_thresh=0.3, min_hole_area=100):
    """
    ...
    return (is_near_image_border(cleaned_mask, border_thickness) or is_occluded_by_others(cleaned_mask, point_map, dilation_iter, z_thresh, filter_size) or
            has_internal_occlusion(cleaned_mask, min_hole_area))
...
def get_mask_renderer(Mask, min_size, Intrinsics, device):
    ...
    cameras = PerspectiveCameras(
        focal_length=torch.tensor([[intrinsics[0, 0], intrinsics[1, 1]]], device=device, dtype=torch.float32),
        principal_point=torch.tensor([[intrinsics[0, 2], intrinsics[1, 2]]], device=device, dtype=torch.float32),
...
def run_alignment(
    ...
    tfm1 = (Transform3d(device=device).scale(scale_1.expand(3)[None]).translate(translation_1[None]))
...
def apply_transform(mesh, center, quat, translation, scale):
    ...
    transformed_mesh = Meshes(verts=[verts], faces=[mesh.faces_packed()], textures=mesh.textures)
...
def compute_loss(rendered, mask_gt, loss_weights, quat, translation, scale):
    ...
    # === Total weighted loss ===
    total_loss = (loss_weights["mask"] * loss_mask + loss_weights["reg_q"] * loss_reg_q + loss_weights["reg_t"] * loss_reg_t +
                  loss_weights["reg_s"] * loss_reg_s)

    return total_loss


def export_transformed_mesh_glb(verts, mesh_obj, center, quat, translation, scale, output_path):
    quat_normalized = quat / quat.norm()
