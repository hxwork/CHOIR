...
from sam3d_objects.pipeline.layout_post_optimization_utils import (apply_transform, check_occlusion, compute_iou, get_mask_renderer, get_mesh, run_alignment,
                                                                   run_ICP, run_render_compare, set_seed)
...
def layout_post_optimization(
    ...
    source_points, target_points, center, tfm1, mesh, ori_iou, final_iou, flag_notgt = (run_alignment(Point_Map, mask, mesh, center, faces_idx, textures,
                                                                                                      renderer, device))
    ...
    if Enable_shape_ICP:
        Flag_ICP = True
        points_aligned_icp, transformation = run_ICP(mesh, source_points, target_points, threshold=0.05)
        ...
        if ori_iou_shapeICP > ori_iou:
            ...
            tfm2 = (Transform3d(device=device).scale(scale_2.expand(3)[None]).translate(translation_2[None]))
    else:
        Flag_ICP = False
        scale_2, translation_2 = torch.tensor(1).to(device), torch.zeros([3]).to(device)
        tfm2 = (Transform3d(device=device).scale(scale_2.expand(3)[None]).translate(translation_2[None]))
    ...
    if not Enable_rendering_optimization:
        ...
        if optimized_iou < 0.5 or optimized_iou <= ori_iou:
            ...
            tfm3 = (
                Transform3d(device=device).translate(-center[None])  # move to center
                .scale(scale.expand(3)[None]).rotate(R.T[None]).translate(center[None])  # move back
...
def pose_decoder(pose_target_convention,):
    ...
    def decode(model_output_dict, scene_scale=None, scene_shift=None):
        ...
        if ("x_instance_rotation_6d" in pose_target_dict or "x_instance_rotation_6d_normalized" in pose_target_dict):
            ...
            if "x_instance_rotation_6d_normalized" in pose_target_dict:
                rot_6d = pose_target_dict["x_instance_rotation_6d_normalized"] * ROTATION_6D_STD.to(
                    pose_target_dict["x_instance_rotation_6d_normalized"].device) + ROTATION_6D_MEAN.to(
        ...
        if "x_translation_scale" in pose_target_dict:
            pose_target_dict["x_translation_scale"] = torch.exp(pose_target_dict["x_translation_scale"])
        ...
        _scene_shift = (scene_shift if scene_shift is not None else torch.tensor([[0, 0, 0]], device=device))
...
def trimesh2o3d_mesh(trimesh_mesh):
    ...
    return o3d.geometry.TriangleMesh(o3d.utility.Vector3dVector(verts), o3d.utility.Vector3iVector(faces))
...
def convert_to_halo(pred_transform, device, dtype):
    on_mesh_transform = Transform3d(dtype=dtype, device=device).rotate(torch.tensor(
        [
            [1, 0, 0],
            [0, 0, 1],
            [0, -1, 0],
    ...
    on_pm_transform = Transform3d(dtype=dtype, device=device).rotate(torch.tensor(
        [
            [-1, 0, 0],
            [0, -1, 0],
            [0, 0, 1],
