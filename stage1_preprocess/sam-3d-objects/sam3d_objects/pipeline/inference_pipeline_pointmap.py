...
from sam3d_objects.pipeline.utils.pointmap import \
...
class InferencePipelinePointMap(InferencePipeline):
        ...
        with torch.inference_mode(False):
            with torch.no_grad():
                for _ in tqdm(range(num_warmup_iters)):
                    ...
                    ss_return_dict = self.sample_sparse_structure(ss_input_dict, inference_steps=None)
    ...
    def _preprocess_image_and_mask_pointmap(self, rgb_image, mask_image, pointmap, img_mask_pointmap_joint_transform):
    ...
    def preprocess_image(
        ...
        preprocessor_return_dict = preprocessor._process_image_mask_pointmap_mess(rgb_image, rgb_image_mask, pointmap)
    ...
    def _clip_pointmap(self, pointmap: torch.Tensor, mask: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        mask_resized = torchvision.transforms.functional.resize(mask, pointmap_size, interpolation=torchvision.transforms.InterpolationMode.NEAREST).squeeze(0)
        ...
        pointmap_clipped_flat = torch.where(pointmap_flat[2, ...].abs() > self.clip_pointmap_beyond_scale * mask_distance,
                                            torch.full_like(pointmap_flat, float('nan')), pointmap_flat)
    ...
    def compute_pointmap(self, image, pointmap=None):
        ...
        if pointmap is None:
            with torch.no_grad():
                with torch.autocast(device_type="cuda", dtype=self.dtype):
                    print(f"pointmap is None, running depth model with shape: {loaded_image.shape}")
                    output = self.depth_model(loaded_image)
            pointmaps = output["pointmaps"]
            camera_convention_transform = (Transform3d().rotate(camera_to_pytorch3d_camera(device=self.device).rotation).to(self.device))
        ...
        if intrinsics is None:
            intrinsics_result = infer_intrinsics_from_pointmap(points_tensor.permute(1, 2, 0), device=self.device)
    ...
    def run_post_optimization(self, mesh_glb, intrinsics, pose_dict, layout_input_dict):
        ...
        revised_quat, revised_t, revised_scale, final_iou, _, _ = (self.layout_post_optimization_method(
            mesh_glb,
            pose_dict["rotation"],
    ...
    def run(
        ...
        image = self.merge_image_and_mask(image, mask)
        with self.device:
            pointmap_dict = self.compute_pointmap(image, pointmap)  # contain pointmap, pts_color, intrinsics
            ori_pointmap = pointmap_dict["pointmap"].clone()
            pointmap = pointmap_dict["pointmap"]
            ...
            return {
                **ss_return_dict,
                **outputs,
                "ori_pointmap": ori_pointmap.cpu().permute((1, 2, 0)),  # HxWx3
                "pointmap": pts.cpu().permute((1, 2, 0)),  # HxWx3
                "pointmap_colors": pts_colors.cpu().permute((1, 2, 0)),  # HxWx3
                "intrinsics": pointmap_dict["intrinsics"],
                "rgb_image_mask": ss_input_dict["rgb_image_mask"],
            }
