...
class Renderer:
    ...
    def __init__(self, cfg: CfgNode, faces: np.array):
        ...
        faces_new = np.array([[92, 38, 234], [234, 38, 239], [38, 122, 239], [239, 122, 279], [122, 118, 279], [279, 118, 215], [118, 117, 215],
                              [215, 117, 214], [117, 119, 214], [214, 119, 121], [119, 120, 121], [121, 120, 78], [120, 108, 78], [78, 108, 79]])
    ...
    def __call__(
        ...
        renderer = pyrender.OffscreenRenderer(viewport_width=image.shape[1], viewport_height=image.shape[0], point_size=1.0)
        material = pyrender.MetallicRoughnessMaterial(metallicFactor=0.0, alphaMode='OPAQUE', baseColorFactor=(*mesh_base_color, 1.0))
        ...
        camera = pyrender.IntrinsicsCamera(fx=self.focal_length, fy=self.focal_length, cx=camera_center[0], cy=camera_center[1], zfar=1e12)
    ...
    def render_rgba_multiple(
        ...
        mesh_list = [
            pyrender.Mesh.from_trimesh(self.vertices_to_trimesh(vvv, ttt.copy(), mesh_base_color, rot_axis, rot_angle, is_right=sss))
        ...
        if color.shape[-1] == 3:
            threshold = 0.0
            alpha_channel = np.where(rend_depth > threshold, 1, 0).astype(np.uint8)
            color = np.concatenate([color, alpha_channel[:, :, np.newaxis]], axis=-1)
        renderer.delete()
