...
from .tools import (camera_marker_geometry, checkerboard_geometry, read_image,
                    transform_pyrender)
...
def init_viewer(
    ...
    if bg_paths is not None:
        bg_imgs = [read_image(p, scale=vis_scale).astype(np.float32) / 255 for p in bg_paths]
...
class AnimationBase(object):
    ...
    def __init__(self, img_size, intrins=None):
        self.scene = pyrender.Scene(ambient_light=[0.3, 0.3, 0.3], bg_color=[1.0, 1.0, 1.0, 0.0])
        ...
        ground = pyrender.Mesh.from_trimesh(make_checkerboard(color0=[0.9, 0.95, 1.0], color1=[0.7, 0.8, 0.85], up="y", alpha=1.0), smooth=False)
    ...
    def add_static_meshes(self, meshes, smooth=True):
        ...
        if num_meshes > num_nodes:
            # need to add a node in the pyrender scene
            self.acquire_lock()
            self.anim_nodes.extend([self.scene.add(self.anim_meshes[0][i]) for i in range(num_nodes, num_meshes)])
        self.release_lock()
    ...
    def add_camera_markers(self, poses):
        ...
        if self.cam_marker_node is None:
            ...
            self.cam_marker_node = self.scene.add(pyrender.Mesh.from_trimesh(cam_marker, smooth=False))
        poses = transform_pyrender(poses)
        poses = np.asarray(poses)
...
class OffscreenAnimation(AnimationBase):
    ...
    def render(self, render_bg=True, render_ground=True, render_cam=True, fac=1.0, render_keypoints=False, **kwargs):
        ...
        if self.check_mesh_visibility():  # if any meshes are visible
            ...
            if rgba.shape[-1] == 3:
                ...
                threshold = 0.0
                alpha_channel = np.where(depth > threshold, 255, 0).astype(np.uint8)
                rgba = np.concatenate([rgba, alpha_channel[:, :, np.newaxis]], axis=-1)
...
class AnimationViewer(AnimationBase):
    ...
    def animate(self, save_name=None, **kwargs):
        ...
        while self.do_animate:
            ...
            if (self.num_repeats > 0 and self.anim_idx >= self.num_repeats * self.anim_len):
                break
...
def make_camera_marker(radius=0.1, height=0.2, up="y", transform=None):
    ...
    if transform is not None:
        ...
        verts = (np.einsum("ij,nj->ni", transform[:3, :3], verts) + transform[None, :3, 3])
