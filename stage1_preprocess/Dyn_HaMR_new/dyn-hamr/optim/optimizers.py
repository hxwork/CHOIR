...
class StageOptimizer(object):
    ...
    def load_checkpoint(self, out_dir, device=None):
        ...
        if os.path.isfile(param_path):
            ...
            # Validate shape compatibility before loading (e.g., seq_len may have changed)
            current_dict = self.model.params.get_dict()
            for k in param_dict:
                if k in current_dict and param_dict[k].shape != current_dict[k].shape:
                    Logger.log(
                        f"Checkpoint shape mismatch for '{k}': "
                        f"saved {tuple(param_dict[k].shape)} vs current {tuple(current_dict[k].shape)}. "
                        f"Ignoring checkpoint and starting fresh."
                    )
                    return
