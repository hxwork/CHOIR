...
from geometry.rotation import (angle_axis_to_rotation_matrix, rotation_matrix_to_angle_axis)
...
class BaseSceneModel(nn.Module):
    ...
    def initialize(self, obs_data, cam_data):
        ...
        if self.use_init and "init_trans" in obs_data:
            ...
            init_trans = (torch.einsum("tij,btj->bti", R_c2w, init_trans + root_loc) + t_c2w[None] - root_loc)
        else:
    ...
    def latent2pose(self, latent_pose):
        ...
        if self.pose_prior is not None:
            ...
            body_pose = rotation_matrix_to_angle_axis(body_pose.reshape((B * T * J_HAND, 3, 3))).reshape((B, T, J_HAND * 3))
    ...
    def pred_params_mano(self, is_right, reproj=True):
        ...
        pred_data = self.pred_mano(self.params.trans, self.params.root_orient, body_pose, is_right, self.params.betas)
