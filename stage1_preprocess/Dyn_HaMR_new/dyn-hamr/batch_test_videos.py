...
Reads CHOIR per-video folders under output/<video_id>/<video_id>.mp4 and writes
Hydra results to output/<video_id>/dynhamr/ (see confs/config.yaml).
"""
import argparse
import logging
...
# CHOIR repo root: stage1_preprocess/Dyn_HaMR_new/dyn-hamr/batch_test_videos.py -> ../../..
...
def process_video(video_path, gpu_id=0, is_static=True, data_root=None, error_log_file=None):
    ...
    # Hydra writes to ${data.root}/${data.video_dir}/dynhamr == output/<id>/dynhamr
...
def main():
    ...
    parser.add_argument(
        ...
        help='CHOIR output root containing <video_id>/<video_id>.mp4 (default: repo output/).',
    )
    ...
    for video_id in subdirs:
        video_path = os.path.join(data_root, video_id, f"{video_id}.mp4")
        if os.path.exists(video_path):
            video_files.append(video_path)

    if args.video_id is not None:
        video_files = [f for f in video_files if Path(f).parent.name in args.video_id]
