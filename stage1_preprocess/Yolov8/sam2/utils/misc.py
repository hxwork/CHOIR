...
def load_video_frames_from_jpg_images(
    ...
    if isinstance(video_path, str) and os.path.isdir(video_path):
        ...
        raise NotImplementedError("Only JPEG frames are supported at this moment. For video files, you may use "
...
def load_video_frames_from_np_array(
    ...
    if isinstance(video_path, np.ndarray):
        ...
        raise NotImplementedError("Only np.ndarray is supported at this moment")

    _, video_height, video_width, _ = images.shape
    img_mean = torch.tensor(img_mean, dtype=torch.float32)[:, None, None]
    img_std = torch.tensor(img_std, dtype=torch.float32)[:, None, None]
