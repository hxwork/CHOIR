...
from pytorch3d.renderer import (MeshRasterizer, PerspectiveCameras, RasterizationSettings, TexturesVertex)
...
def get_moge_pointcloud(image_tensor, device='cuda'):
    ...
    moge_model = MoGeModel.from_pretrained("Ruicheng/moge-vitl/model.pt").to(device)
    moge_model.eval()
    with torch.no_grad():
...
def denormalize_intrinsics(norm_K, height, width):
    ...
    return np.array([[fx_abs, 0.0, cx_abs], [0.0, fy_abs, cy_abs], [0.0, 0.0, 1.0]])
...
def crop_mesh_with_mask(vertices, faces, focal_length, mask, device='cuda'):
    ...
    camera = PerspectiveCameras(focal_length=((fx, fy),), principal_point=((cx, cy),), image_size=((H, W),), in_ndc=False, device=device)
...
def process_and_save_alignment(mesh_path, mask_path, image_path, output_dir, device='cuda', focal_length_json_path=None):
    ...
    try:
        ...
        # Process alignment
        result = process_3db_alignment(mesh_path=mesh_path,
                                       mask_path=mask_path,
        ...
        if result is not None:
            ...
            aligned_mesh = trimesh.Trimesh(vertices=result['aligned_vertices_opengl'], faces=result['faces'])
...
def visualize_meshes_interactive(aligned_mesh_path, dfy_mesh_path, output_dir=None, share=True, height=600):
    ...
    try:
        ...
        with gr.Blocks() as demo:
            ...
            gr.Model3D(value=combined_glb_path, label="Combined 3D Scene (Interactive)", height=height)
...
def visualize_meshes_comparison(aligned_mesh_path, dfy_mesh_path, use_interactive=False):
    ...
    try:
        ...
        ax.scatter(dfy_mesh.vertices[:, 0], dfy_mesh.vertices[:, 1], dfy_mesh.vertices[:, 2], c='blue', s=0.1, alpha=0.6, label='3Dfy Original')
