...
# CHOIR repo root: stage1/sam-3d-objects/run_reconstruction.py -> ../..
...
def process_video_directory(video_dir, inference_model, output_root_dir, post_optimize, args):
    ...
    output_video_dir = os.path.join(output_root_dir, video_id)
    os.makedirs(output_video_dir, exist_ok=True)

    image_path = os.path.join(video_dir, "rgbs", "0.png")
    ...
    mask_path = os.path.join(video_dir, "obj_masks", "0.png")
    print(f"Processing {mask_path}...")
    image_np_full = load_image(image_path)
    mask_np = load_mask(mask_path)

    mask_idx = 0
    glb_path = os.path.join(output_video_dir, f"glb_{mask_idx}.glb")
    intrinsics_path = os.path.join(output_video_dir, "intrinsics.json")
    ...
    output_depth_dir = os.path.join(output_video_dir, "env_depths")
    ...
    for item in outputs_data:
        ...
        transform_info_path = os.path.join(output_video_dir, f"transform_{mask_idx}.json")
    ...
    output_render_path = os.path.join(output_video_dir, "rendered_on_image.png")
