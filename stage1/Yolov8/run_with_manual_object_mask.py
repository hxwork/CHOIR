...
# OUTPUT_DATA_DIR : where per-video subfolders with rgbs/, obj_masks/, rh_masks/,
#                   annotations/, visualizations/, JSON files, and the cropped
#                   {video_id}.mp4 are written for downstream tools.
# ---------------------------------------------------------------------------
...
def process_video_with_visualization(video_path, hand_det_model, sam_predictor, sam_video_predictor, conf,
    ...
    try:
        ...
        sam3d_video_path = output_data_dir / video_name / f"{video_name}.mp4"
...
def build_arg_parser():
    ...
    parser.add_argument('--output_dir', type=str, default=DEFAULT_OUTPUT_DATA_DIR,
                        help=f'Root for downstream artifacts: per-video subfolders with rgbs/, '
                             f'obj_masks/, rh_masks/, lh_masks/, json files, and the (re-encoded) '
                             f'video (default: {DEFAULT_OUTPUT_DATA_DIR})')
