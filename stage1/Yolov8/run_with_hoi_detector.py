...
def process_video_with_visualization(video_path, hand_det_model, sam_predictor, sam_video_predictor, conf, output_data_dir, visualize=True):
    ...
    try:
        ...
        # --- Save all RGB frames ---
        output_rgb_folder = output_data_dir / video_name / 'rgbs'
        ...
        if rh_bboxes:
            with open(output_folder / 'rh_bbox.json', 'w') as f:
                json.dump(rh_bboxes, f, indent=4)
        if lh_bboxes:
            with open(output_folder / 'lh_bbox.json', 'w') as f:
                json.dump(lh_bboxes, f, indent=4)

        # --- Save hand keypoints ---
        if rh_keypoints:
            with open(output_folder / 'rh_keypoints.json', 'w') as f:
                json.dump(rh_keypoints, f, indent=4)
        if lh_keypoints:
            with open(output_folder / 'lh_keypoints.json', 'w') as f:
                json.dump(lh_keypoints, f, indent=4)

        # --- Save RGBA masks for OBJECT ---
        output_seg_mask_folder = output_data_dir / video_name / 'obj_masks'
        ...
        if right_hand_video_segments:
            rh_mask_folder = output_data_dir / video_name / 'rh_masks'
        ...
        if left_hand_video_segments:
            lh_mask_folder = output_data_dir / video_name / 'lh_masks'
...
def process_video_worker(args):
    ...
    if 'sam_predictor' not in _WORKER_MODELS:
        sam2_checkpoint = "/vepfs_default/chanxueyan/lhp/xh/code/sam2-main/checkpoints/ckpts/sam2.1_hiera_large.pt"
...
def sample_videos(num_samples=1000, output_dir=DEFAULT_OUTPUT_DATA_DIR):
    """
    Randomly samples videos from the taste_rob dataset, but only those classified as
    'single-hand-single-object' with a 'rigid' object based on llm_output.json.
    """
    # --- Step 1: Get target video IDs from llm_output.json ---
    json_path = 'llm_output.json'
    ...
    try:
        ...
        for item in data:
            if item.get('interaction_type') == 'single-hand-single-object':
                objects = item.get('objects', [])
                if any(obj.get('type') == 'rigid' for obj in objects):
                    video_id = item.get('video_id')
                    if video_id:
                        target_video_ids.add(video_id)

    except FileNotFoundError:
        print(f"Error: The file {json_path} was not found. Cannot determine which videos to sample.")
        return []
    except json.JSONDecodeError:
        print(f"Error: Could not decode JSON from {json_path}. Cannot determine which videos to sample.")
        return []

    if not target_video_ids:
        print(f"Found 0 videos matching 'single-hand-single-object' with rigid objects in {json_path}.")
        return []

    print(f"Found {len(target_video_ids)} target video IDs from {json_path}.")

    # --- Step 2: Find matching video files in the dataset ---
    base_dir = Path("/jiigan-hp/xh/data/taste_rob")

    # Collect all video paths
    all_videos = []
    # for hand_type in ['DoubleHand', 'SingleHand']:
    for hand_type in ['SingleHand']:
    ...
    print(f"Found {len(all_videos)} total videos in the dataset directory.")

    # Filter videos based on the target IDs
    filtered_videos = [video for video in all_videos if video.stem in target_video_ids]

    if not filtered_videos:
        print("Could not find any video files matching the target IDs.")
        return []

    print(f"Found {len(filtered_videos)} matching video files to sample from.")

    # --- Step 3: Sample from the filtered list ---
    num_samples = min(num_samples, len(filtered_videos))
    random.seed(1234)  # modified for final submission
    sampled_video_paths = random.sample(filtered_videos, num_samples)
    sampled_video_paths = sorted(sampled_video_paths, key=lambda x: int(x.stem))
    print(f"Successfully sampled {num_samples} videos.")

    success_count = 0
    output_dir = Path(output_dir)
    for idx, video_path in enumerate(sampled_video_paths, 1):
        video_id = video_path.stem
        video_name = video_path.name
        output_path = output_dir / video_id / f"{video_id}.mp4"
        output_path.parent.mkdir(parents=True, exist_ok=True)

        print(f"[{idx}/{num_samples}] Copying {video_name}...")

        try:
            # Copy video file
            shutil.copy2(video_path, output_path)
            print(f"Successfully copied {video_name} to {output_path}")
            success_count += 1

        except Exception as e:
            print(f"Failed to copy {video_name} to {output_path}: {e}")
            continue

    print(f"\n{'='*60}")
    print(f"Completed! Successfully copied {success_count}/{num_samples} videos")
...
def build_arg_parser():
    ...
    parser.add_argument('--data_source',
                        type=str,
                        default='local',
                        choices=['taste_rob', 'local', 'my_data'],
                        help='Data source to use: "taste_rob" samples from the Taste-Rob dataset, '
                        '"local" loads flat videos from --data_dir; "my_data" is a legacy alias '
                        '(default: local)')
    parser.add_argument('--num_samples', type=str, default='1', help='Number of videos to sample (taste_rob only), or "all" (default: 1)')
    ...
    parser.add_argument('--output_dir', type=str, default=str(DEFAULT_OUTPUT_DATA_DIR),
                        help=f'Root for downstream per-video artifacts (default: {DEFAULT_OUTPUT_DATA_DIR})')
...
def main():
    ...
    # Parse num_samples (only relevant for taste_rob)
    ...
    print("=" * 80)
    if args.data_source in {'local', 'my_data'}:
        ...
        print("Data source: taste_rob (sampling from Taste-Rob dataset)")
        print("=" * 80)
        videos_to_process = sample_videos(num_samples=num_samples, output_dir=args.output_dir)
        if not videos_to_process:
