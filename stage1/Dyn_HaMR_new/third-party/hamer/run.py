...
def extract_raw_bboxes_from_keypoints_json(img_paths, sequence_name, hand_type='right', vis_dir=None):
    """
    Pass 1: Extract raw hand bboxes from pre-existing keypoints JSON file.
    读取已有的关键点JSON文件，从关键点坐标计算bbox。
    ...
    # Build JSON path from CHOIR video folder: output/<id>/dynhamr/images -> output/<id>/
    video_dir = Path(img_paths[0]).resolve().parents[1]
    keypoints_file = video_dir / f"{hand_type[0]}h_keypoints.json"
    ...
    for frame_idx, img_path in enumerate(tqdm(sorted(img_paths), desc="Pass 1: Computing bboxes from keypoints")):
        ...
        if frame_key in keypoints_data:
            ...
            # 添加置信度列（全部设为1.0，表示关键点有效）
            keypoints_with_conf = np.zeros((21, 3))
            keypoints_with_conf[:, :2] = keypoints_2d
            keypoints_with_conf[:, 2] = 1.0  # 置信度

            # 从关键点计算bbox（使用1.2倍缩放使bbox稍大一些）
