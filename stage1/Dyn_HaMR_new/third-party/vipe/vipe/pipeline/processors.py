...
class IntrinsicEstimationProcessor(StreamProcessor):
    ...
    def __call__(self, frame_idx: int, frame: VideoFrame) -> VideoFrame:
        ...
        frame.intrinsics = torch.as_tensor([fx, fy, frame_width / 2, frame_height / 2] + self.distortion,).float()
...
class GeoCalibIntrinsicsProcessor(IntrinsicEstimationProcessor):
    ...
    def __init__(
        ...
        #         camera_model=camera_model,x
        #     )

        # self.fov_y = res["camera"].vfov[0].item()
        self.camera_type = camera_type
        logger.info(f"Load intrinsics from {video_stream.path.parent/'intrinsics.json'}")
        with open(video_stream.path.parent / "intrinsics.json") as f:
            res = json.load(f)['intrinsics']
            vfov = np.arctan2(video_stream._height / 2, res[1][1]) * 2
        self.fov_y = vfov
    ...
    def update_iterator(self, previous_iterator: Iterator[VideoFrame]) -> Iterator[VideoFrame]:
        ...
        for frame_idx, frame in pbar(enumerate(data_iterator), desc="Aligning depth"):
            ...
            if min_uv_score < 0.3:
                ...
                prompt_result = self.depth_model.estimate(DepthEstimationInput(rgb=frame.rgb.float().cuda(), focal_length=focal_length)).metric_depth
                ...
                prompt_result = self.prompt_model.estimate(DepthEstimationInput(
