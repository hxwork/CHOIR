"""
Pipeline that combines two steps:
  Step 1 – prepare_sam3d:   Load sam-3d-objects meshes, apply vertex transform,
                             and write decomposed.obj + scale.json into meshdata/sam3d/.
  Step 2 – fps_sampling:    FPS-sample point clouds (with normals) from every
                             decomposed.obj found under meshdata/ and export PLY files.

Use --video_id to restrict both steps to specific IDs (default: all).
Either step can be skipped with --skip_prepare / --skip_sampling.
"""
import argparse
import json
import math
...
def prepare_sam3d(source_base_dir: Path, dest_base_dir: Path, video_ids=None):
    """
    从 sam-3d-objects 源目录读取数据，对每个 video_id:
      - obj_canonical.obj  -> 顶点变换 (x,y 取反) -> decomposed.obj
      - obj_00000.json     -> 提取 "scale" 字段   -> scale.json
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source_dir",
        type=str,
        default="/vepfs_default/chanxueyan/lhp/xh/code/sam-3d-objects/input_data/",
        help="Source base directory for sam-3d-objects.",
    )
    parser.add_argument(
        "--video_id",
        nargs="+",
    ...
    parser.add_argument(
        ...
        help="Root directory for meshdata (used by both steps).",
    )
