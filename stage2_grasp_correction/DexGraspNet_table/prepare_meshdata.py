"""
Prepare object meshes for Stage 2 (inference / upstream mesh packaging).

Step 1 – prepare_sam3d: load HOI meshes (stage3/intermediates/optimized_hoi_seq),
                         apply vertex transform, write decomposed.obj + scale.json
                         into meshdata/sam3d/.
Step 2 – fps_sampling:   FPS-sample point clouds (with normals) and export PLY files.

Not the training grasp-generation entry; that is grasp_generation/main_prep_data.py.
Use --video_id to restrict both steps (default: all). Skip either step with
--skip_prepare / --skip_sampling.
"""
import argparse
import json
import math
...
DEFAULT_SOURCE_DIR = str(REPO_ROOT / "output")
...
def prepare_sam3d(source_base_dir: Path, dest_base_dir: Path, video_ids=None):
    """
    Read sam-3d / HOI fitting outputs and write meshdata/sam3d/<video_id>/:
    ...
    for video_dir in tqdm(video_dirs, desc="[Step 1] preparing sam3d meshes"):
        ...
        seq_dir = video_dir / "stage3" / "intermediates" / "optimized_hoi_seq"
...
def fps_sampling(data_root: str, video_ids=None):
    ...
    dexgraspnet_root = os.path.join(data_root, "dexgraspnet")
    if not video_ids:
        print("[Step 2] no video_id given, discovering all DexGraspNet objects in data_root ...")
        object_dirs = []
        if os.path.isdir(dexgraspnet_root):
            for name in os.listdir(dexgraspnet_root):
                d = os.path.join(dexgraspnet_root, name)
                if os.path.isdir(d):
        ...
        object_dirs = []
        for vid in video_ids:
            d = os.path.join(dexgraspnet_root, vid)
            if os.path.isdir(d):
                object_dirs.append(d)
            else:
                print(f"[Step 2] warning: object_id '{vid}' not found in meshdata/dexgraspnet.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Source root with <video_id>/stage3/intermediates/optimized_hoi_seq/ "
             "(default: CHOIR output/).",
    )
    parser.add_argument(
        "--video_id",
        nargs="+",
    ...
    parser.add_argument(
        ...
        help="Root directory for meshdata (default: stage2_grasp_correction/DexGraspNet_table/meshdata).",
    )
