...
if __name__ == '__main__':
    ...
    # Optional output path; default is the current directory
    ...
    # load results
    data_path = os.path.join(args.result_path, args.object_code + '.npy')
    if not os.path.exists(data_path):
        print(f"Error: Result file not found at {data_path}")
        exit()

    data_dict = np.load(data_path, allow_pickle=True)[args.num]
    plane = torch.tensor(data_dict['plane'], dtype=torch.float, device=device)  # plane parameters
    pose = plane2pose(plane)  # 4x4 homogeneous transformation matrix
    qpos = data_dict['qpos']
    ...
    # object model
    object_model = ObjectModel(data_root_path='../data/meshdata', batch_size_each=1, num_samples=2000, device=device)
    ...
    if 'energy' in data_dict:
        ...
        result_text = f'Index {args.num}  scale {scale}  E_fc {E_fc}  E_dis {E_dis}  E_pen {E_pen}  E_prior {E_prior}  E_spen {E_spen}  E_tpen {E_tpen}'
    ...
    save_filename = f"vis_{args.object_code}_{args.num}.html"
