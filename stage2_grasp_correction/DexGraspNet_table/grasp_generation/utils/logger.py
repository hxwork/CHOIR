...
class Logger:
    ...
    def log(self, energy, E_fc, E_dis, E_pen, E_prior, E_spen, E_tpen, E_anatomy, step, show=False):
        """
        ...
        self.writer.add_scalar('Energy/anatomy', E_anatomy.mean(), step)
        ...
        if show:
            print(f'Step %d energy: %f  fc: %f  dis: %f  pen: %f  anatomy: %f' % (step, energy.mean(), E_fc.mean(), E_dis.mean(), E_pen.mean(), E_anatomy.mean()))
