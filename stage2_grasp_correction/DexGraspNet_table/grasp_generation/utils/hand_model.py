...
class HandModel:
    ...
    def get_plotly_data(self, i, opacity=0.5, color='lightblue', with_keypoints=False, with_contact_points=False, pose=None):
        ...
        if with_contact_points:
            ...
            hand_plotly.append(
                go.Scatter3d(x=contact_points[:, 0], y=contact_points[:, 1], z=contact_points[:, 2], mode='markers', marker=dict(color='red', size=5)))
        return hand_plotly
...
class HandMineModel:
    ...
    def __init__(self, mano_root, contact_indices_path, pose_distrib_path, device='cpu'):
        """
        Create a Hand Mine Model for MANO
        ...
        self.device = device
        self.manolayer = ManoLayer(mano_assets_root=mano_root, flat_hand_mean=True, use_pca=False, side='right').to(device=self.device)
        self.axisFK = AxisLayerFK(side=self.manolayer.side, mano_assets_root=mano_root).to(device=self.device)
    ...
    def set_parameters(self, hand_pose, contact_point_indices=None):
        ...
        if self.hand_pose.requires_grad:
            self.hand_pose.retain_grad()
        self.mano_output = self.manolayer.forward(self.hand_pose[:, 3:])
        self.vertices = self.mano_output.verts
        self.keypoints = self.mano_output.joints
        self.vertices = self.vertices + self.hand_pose[:, :3].unsqueeze(1)
        self.keypoints = self.keypoints + self.hand_pose[:, :3].unsqueeze(1)
        self.contact_point_indices = contact_point_indices
        self.contact_points = self.vertices[torch.arange(len(hand_pose)).unsqueeze(1), self.contact_indices[self.contact_point_indices]]
    ...
    def anatomy(self):
        """
        Calculate anatomy energy
        
        Returns
        -------
        E_anatomy: (N,) torch.Tensor
            anatomy energy
        """
        T_g_p = self.mano_output.transforms_abs  # (B, 16, 4, 4)
        T_g_a, _R, ee = self.axisFK(T_g_p)  # ee (B, 16, 3)
        loss = self.anatomyLoss(ee)
        loss = loss.sum(dim=-1)
        return loss
