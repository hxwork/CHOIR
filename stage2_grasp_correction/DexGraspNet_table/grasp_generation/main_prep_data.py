...
`--data_root/dexgraspnet/<id>/` to contain at least:
  - decomposed.obj
  - init_obj_poses.npy
...
if __name__ == '__main__':
    ...
    if not args.object_code_list:
        ...
        all_object_dirs = []
        source_path = os.path.join(args.data_root, 'dexgraspnet')
        ...
        for code in args.object_code_list:
            obj_dir = os.path.join(args.data_root, 'dexgraspnet', code)
            if os.path.isdir(obj_dir):
                all_object_dirs.append(obj_dir)
            else:
                print(f"Warning: object code {code} not found in dexgraspnet directory.")
        args.object_code_list = all_object_dirs
