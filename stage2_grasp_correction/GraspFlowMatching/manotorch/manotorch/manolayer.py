...
class ManoLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        **kargs,
        ...
        assert os.path.isfile(mano_assets_path), f"Can not find MANO assets {mano_assets_path}, please follow steps in README.md"
    ...
    def skinning_layer(self, full_rots: torch.Tensor, betas: Optional[torch.Tensor]):
        ...
        T_P_homo = torch.cat([T_P.transpose(2, 1), torch.ones((batch_size, 1, B_P.shape[1]), dtype=T.dtype, device=T.device)], dim=1)
