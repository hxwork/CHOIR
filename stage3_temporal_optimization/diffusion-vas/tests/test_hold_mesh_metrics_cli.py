...
SCRIPT_PATH = REPO_ROOT / "compute_hold_mesh_metrics.py"


class HoldMeshMetricsCliTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_cli_does_not_expose_fps(self):
        flag_names = {
            arg.value
            for node in ast.walk(self.tree)
            if isinstance(node, ast.Call)
    ...
    def test_metric_keys_are_declared(self):
        ...
        self.assertIn("METRIC_KEYS", assigned_names)
