"""CHOIR Stage 3 release entry: temporal HOI fitting with SAM3D object tracking.

Includes PnP health dry-run diagnostics and interaction motion profiling.
Segmented SAM3D reset/bridge logic was removed from this release tree.
"""
...
from choir_stage3.geometry.contact_constraint import (BUCKET_INSIDE, BUCKET_NEAR_CONTACT,
...
from choir_stage3.vision.contact_map_estimator import estimate_contact_map_for_sampled_frames, load_contact_indices
from choir_stage3.vision.debug_bbox import get_global_amodal_bbox, load_hand_data, load_raw_frames
from choir_stage3.optimization.frame_sampling import build_adaptive_sample_indices
from choir_stage3.io.gfm_offset_application import apply_camera_ray_depth_offsets
from choir_stage3.io.gfm_preprocess import export_graspflowmatching_sequence
from choir_stage3.io.hand_input_selection import resolve_hand_inputs
from choir_stage3.vision.hand2d_anatomy_gate import compute_anatomy_hand2d_frame_weights
from models.diffusion_vas.pipeline_diffusion_vas import DiffusionVASPipeline
from choir_stage3.optimization.motion_policy import FORCE_ROTATION_MODE, force_rotation_likely_profile
from choir_stage3.io.hoi_io import (
    save_hoi_sequence,
    save_k3d_visualization,
)
...
from choir_stage3.geometry.pnp import compute_pnp_health, flag_reset_candidates, generate_queries, run_pnp_1stage
from choir_stage3.geometry.rendering import (get_render_params, render_side_view_rgb, vectorized_pose_guiding_loss, weighted_false_negative_loss, weighted_false_positive_loss)
from choir_stage3.optimization.stage3_hold_policy import (
...
from choir_stage3.optimization.stage3_rotation_protection import (
    rotation_drift_degrees,
    stage3_rotation_lr_for_step,
    stage3_rotation_smoothness_scale,
)
from choir_stage3.optimization.stage5_mode import (
    STAGE5_FROZEN,
    STAGE5_MODES,
    STAGE5_OBJECT_LITE,
    STAGE5_POSE_ONLY,
...
from choir_stage3.optimization.stage5_object_lite import (
    DEFAULT_OBJECT_LITE_MAX_TRANS_DELTA,
    project_object_translation_delta_,
)
from choir_stage3.optimization.stage5_ray_delta import (
    DEFAULT_STAGE5_RAY_MAX_DELTA,
    DEFAULT_STAGE5_RAY_MIN_DELTA,
    build_stage5_ray_delta,
    clamp_stage5_ray_delta_,
)
from choir_stage3.optimization.stage_layout_detection import (
    LAYOUT_FIVE_STAGE,
...
MODULE_ROOT = Path(__file__).resolve().parents[2]  # diffusion-vas/
REPO_ROOT = MODULE_ROOT.parents[1]
STAGE1_ROOT = REPO_ROOT / "stage1_preprocess"
STAGE2_ROOT = REPO_ROOT / "stage2_grasp_correction"
GFM_ROOT = STAGE2_ROOT / "GraspFlowMatching"
DEX_ROOT = STAGE2_ROOT / "DexGraspNet_table"
DEFAULT_INPUT_ROOT = REPO_ROOT / "output"
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "output"
DEFAULT_MANO_DATA_ROOT = STAGE1_ROOT / "Dyn_HaMR_new" / "_DATA" / "data"
DEFAULT_GFM_CHECKPOINT = GFM_ROOT / "results" / "050-Linear-velocity-None" / "checkpoints" / "0040000.pt"
DEFAULT_CONTACT_INDICES = DEX_ROOT / "grasp_generation" / "mano" / "contact_indices.json"
from choir_stage3.geometry.utils import *
from choir_stage3.sam3d.sam3d_reset import calculate_iou
from choir_stage3.sam3d.sam3d_ref_masks import load_sam3d_ref_obj_mask
from choir_stage3.sam3d.sam3d_sparse_keyframes import (
...
def find_static_end(masks_np, iou_threshold=0.9):
    """
    Find the end of a static segment (first frame that starts moving).
    
    Args:
        masks_np: (N, H, W) numpy array of binary masks
        iou_threshold: IoU above this means the object is considered static
    
    Returns:
        static_end: exclusive end index of the static segment (first moving frame)
    """
    ...
    # Find the first non-static frame
    static_end = 1  # keep at least the first frame
    for i in range(len(is_static)):
        if not is_static[i]:
            static_end = i + 1  # +1 because is_static[i] compares frames i and i+1
            break

    # If entirely static, return the last frame index
...
def detect_static_segments(masks_np, iou_threshold=0.9):
    """
    Detect static segments at the start (0-1) and end (4-5) of the sequence.
    By default the first and last frames are treated as static.
    ...
    Returns:
        start_static_end: exclusive end index of the opening static segment
        end_static_start: inclusive start index of the closing static segment
    """
    num_frames = len(masks_np)

    # Opening static segment (0-1): forward scan
    start_static_end = find_static_end(masks_np, iou_threshold)

    # Closing static segment (4-5): reverse scan
    ...
    if start_static_end >= end_static_start:
        # On failure, default to first/last quarter
...
def init_metric_depth_model():
    ...
    # Newer moge/huggingface_hub reject "namespace/repo/file.pt" as a repo id.
    # Prefer a local weight; fall back to the official HF repo id.
    local_candidates = [
        os.environ.get("CHOIR_MOGE_CHECKPOINT", ""),
        str(MODULE_ROOT / "checkpoints" / "moge-v2-vitl-normal" / "model.pt"),
        str(STAGE1_ROOT / "sam-3d-objects" / "Ruicheng" / "moge-v2-vitl-normal" / "model.pt"),
        str(MODULE_ROOT / "Ruicheng" / "moge-v2-vitl-normal" / "model.pt"),
    ]
    pretrained = next((p for p in local_candidates if p and Path(p).is_file()), "Ruicheng/moge-2-vitl-normal")
    metric_depth_model = MoGeModel.from_pretrained(pretrained).to(device)
...
def parse_per_frame_contact_map(contact_list, sampled_indices):
    """
    Parse a per-frame contact map (current JSON format):
    [{"frame_index": int, "correspondences": {"hand_vtx": {"face_id": int, "bary_coords": [...]}}}, ...]

    frame_index is an absolute video frame id, mapped into sampled_indices.
    Keep only frames that appear in sampled_indices.

    Returns:
        dict: {sampled_pos (int): {"hand_idx": Tensor, "face_vert_idx": Tensor, "bary": Tensor}}
        where sampled_pos is the index of that frame inside sampled_indices.
    """
    if not contact_list:
        return {}
...
def run_sam3d_ref_keyframe_init(
    *,
    ...
    from choir_stage3.sam3d.sam3d_reset import (
        SAM3D_CONFIG_PATH,
        _SAM3D_AVAILABLE,
        _SAM3D_IMPORT_ERROR,
...
class TemporalHandObjectPose(nn.Module):
    ...
    def __init__(self,
        ...
        mano_dir = str(DEFAULT_MANO_DATA_ROOT)
        amano_cfg = {
...
def fit_and_visualize_pose(
    ...
    layout = VideoLayout.from_root(output_path)
    layout.ensure_stage_dirs()
    _diag = str(layout.stage3_diagnostics_dir)
    _logs = str(layout.stage3_logs_dir)
    _prev_v = str(layout.stage3_preview_videos_dir)
    _prev_i = str(layout.stage3_preview_images_dir)
    _prev_m = str(layout.stage3_preview_meshes_dir)
    with open(os.path.join(_diag, "sampling_policy.json"), "w") as _spf:
    ...
    ref_policy_path = os.path.join(_diag, "sam3d_reference_frame.json")
    ...
    transform_path = str(layout.transform_json)
    ...
    if sam3d_ref_keyframe_init:
        ...
        with open(os.path.join(_diag, "sam3d_ref_keyframe_init.json"), "w") as _skf:
    ...
    # ========================================================================
    # Checkpoint: save/load Stage 3 results to skip re-running Stages 1-3
    #   - First run: execute Stages 1-3 and save a checkpoint at the end
    #   - Later runs with contact maps: load the checkpoint and jump to Stage 4+
    # ========================================================================
    checkpoint_path = str(layout.stage3_checkpoint)
    _ckpt_loaded = False
    ...
    if os.path.exists(checkpoint_path) and checkpoint_is_usable:
        ...
        if sam3d_sparse_keyframes:
            ...
            for _fn in (
                os.path.join("stage3", "diagnostics", "interaction_motion_profile_sam3d_gate.json"),
                os.path.join("stage3", "diagnostics", "sam3d_policy_diag", "interaction_motion_profile_sam3d_gate.json"),
                os.path.join("stage3", "diagnostics", "interaction_motion_profile_stage3.json"),
            ):
    ...
    if not _ckpt_loaded:
        ...
        def _run_stage1():
            ...
            stage_ranges_path = os.path.join(_diag, "stage_frame_ranges.json")
            with open(stage_ranges_path, 'w') as f:
                json.dump(stage_ranges, f, indent=4)
            print(f"Saved stage frame ranges to {stage_ranges_path}")

            stage_layout_debug_path = os.path.join(_diag, "stage_layout_debug.json")
        ...
        def _run_stage2():
            ...
            _log_path = os.path.join(_logs, "optimize_stage1_3.log")
            ...
            for step in loop:
                ...
                # Apply the same pose to all frames in the opening static segment
                posed_mesh = single_frame_model()

                # Replicate the mesh into the batch
            ...
            if first_target_points is not None and first_target_points.shape[0] > 10:
                ...
                # result.RT is (1, 4, 4): transform that aligns Source to Target
                ...
                pre_debug_ply_path = os.path.join(_prev_m, f"stage1_debug_icp_pre_alignment_with_mesh_{first_frame_idx}.ply")
                ...
                debug_ply_path = os.path.join(_prev_m, f"stage1_debug_icp_post_alignment_{first_frame_idx}.ply")
            ...
            for vis_idx in vis_indices:
                ...
                save_path = os.path.join(_prev_i, f"stage1_start_static_frame_{vis_idx}.png")
        ...
        def _run_stage3():
            ...
            try:
                from choir_stage3.optimization.interaction_motion_profile import build_interaction_motion_profile
                ...
                if (approaching_end_idx is not None and interaction_end_idx is not None
                    ...
                    _imp3s = os.path.join(_diag, "interaction_motion_profile_stage3.json")
            ...
            def _emit_interaction_policy_diagnostics():
                ...
                _diag_dir = os.path.join(_diag, "sam3d_policy_diag")
            ...
            # Single-shot PnP. Segmented SAM3D reset/bridge was removed from this release.
            ...
            pnp_forward = run_pnp_1stage(
                ...
                optimized_T,
                device,
                output_dir=os.path.join(_diag, "pnp_visualization_forward" if sam3d_ref_enabled else "pnp_visualization"),
                vis_threshold=0.0)
            ...
            if sam3d_ref_enabled:
                ...
                if pnp_start_frame_idx > 0:
                    pnp_backward_rev = run_pnp_1stage(
                        ...
                        optimized_T,
                        device,
                        output_dir=os.path.join(_diag, "pnp_visualization_backward"),
                        vis_threshold=0.0)
            ...
            try:
                ...
                if (approaching_end_idx is not None and interaction_end_idx is not None
                    ...
                    _imp3_path = os.path.join(_diag, "interaction_motion_profile_post_pnp.json")
            ...
            if pnp_health_dump:
                try:
                    ...
                    _csv_path = os.path.join(_diag, "pnp_health.csv")
                    ...
                    _json_path = os.path.join(_diag, "pnp_health.json")
                    with open(_json_path, "w") as _jf:
                        json.dump({
            ...
            if _hold_stage3:
                ...
                _mano_mean_path = str(DEFAULT_MANO_DATA_ROOT / "mano" / "MANO_RIGHT.pkl")
            ...
            _s3pre_video_path = os.path.join(_prev_v, "stage3_pre_optim.mp4")
            ...
            sparse_video_path = os.path.join(_prev_v, "optimized_fitting_SPARSE.mp4")
        ...
        try:
            ...
            if (approaching_end_idx is not None and interaction_end_idx is not None
                ...
                _gate_json = os.path.join(_diag, "interaction_motion_profile_sam3d_gate.json")
        ...
        if sam3d_sparse_keyframes and sam3d_gate_suggested_mode == "rotation_likely":
            ...
            if _onset >= num_sampled_frames:
                ...
                _cmp_dir = os.path.join(_diag, "sam3d_sparse_keyframes", "rotation_dense", "pnp_vs_sam3d_vis")
        ...
        # ----------------------------------------------------------------
        # Save Stage 3 checkpoint (later Stage 7-8 can resume from here)
        # ----------------------------------------------------------------
    ...
    def _run_stage4():
        ...
        # No optimisation, no log file. Estimates a per-frame contact map
        # in-memory for use by STAGE 5/6.
        # ========================================================================
        print("\n" + "=" * 80)
        print("STAGE 4: Loading Contact Correspondence")
        print("=" * 80)

        grasp_correction_dir = str(layout.grasp_correction_dir)
        camera_ray_depth_offset_path = os.path.join(grasp_correction_dir, "camera_ray_depth_offset.json")

        _stage4_camera_offset_enabled = not bool(no_stage4_camera_offset)

        if _stage4_camera_offset_enabled and overwrite_grasp_correction and os.path.exists(camera_ray_depth_offset_path):
        ...
        if _stage4_camera_offset_enabled and not os.path.exists(camera_ray_depth_offset_path):
            # Auto-run the GraspFlowMatching pre-pass to generate the JSON using
            # the same Python environment as this script. Two commands are run
            # sequentially inside stage2_grasp_correction/GraspFlowMatching:
            ...
            _gfm_dir = str(GFM_ROOT)
            _video_id = os.path.basename(seq_path)
            _gfm_seq_dir_name = "stage2/grasp_correction/gfm_input_hoi_seq"
            ...
            _cmd_prep = [sys.executable,
                         "prepare_meshdata.py",
                         "--video_id", _video_id,
                         "--seq_dir_name", _gfm_seq_dir_name]
            _cmd_sample = ["torchrun",
                           "--nnodes=1", "--nproc_per_node=1",
                           f"--master_port={_master_port}",
                           "sample_cam_ray_ddp.py", "ODE",
                           "--ckpt", str(DEFAULT_GFM_CHECKPOINT.relative_to(GFM_ROOT)),
        ...
        if _stage4_camera_offset_enabled:
            ...
            print("Stage 4 camera-ray depth offset disabled by --no_stage4_camera_offset")
        ...
        with torch.no_grad():
            ...
            if not _stage4_camera_offset_enabled:
                with open(os.path.join(grasp_correction_dir, "camera_ray_depth_offset_apply_debug.json"), "w") as _dbg_f:
                    json.dump(
                        {
                            "disabled": True,
                            "reason": "no_stage4_camera_offset",
                            "applied_count": 0,
                            "total_count": int(len(sampled_indices)),
                        },
                        _dbg_f,
                        indent=2,
                    )
    ...
    _reclassify_log_path_s5 = os.path.join(_logs, "optimize_stage5_reclassify.log")
    _prefix_diag_path_s5 = os.path.join(_logs, "optimize_stage5_prefix_diag.csv")
    ...
    def _run_stage5():
        ...
        # ================================================================
        # STAGE 5 — Penetration Resolution (primary: penetration; secondary: contact)
        # ================================================================
        ...
        _log_path_s5 = os.path.join(_logs, "optimize_stage5.log")
        ...
        with open(os.path.join(_diag, "stage5_mode_debug.json"), "w") as _f:
            json.dump({
        ...
        if _stage5_pose_ray and _stage5_ray_delta is not None:
            ...
            with open(os.path.join(_diag, "stage5_ray_delta_debug.json"), "w") as _f:
        ...
        if _has_inter:
            _dump_dir_s5 = os.path.join(_diag, "stage5_debug")
    ...
    def _run_stage6():
        # ================================================================
        # STAGE 6 — Contact Tightening (primary: contact; secondary: pen + bary smoothing)
        # ================================================================
        ...
        _log_cols_s6 = ["step", "total", "pen", "contact",
                        ...
                        "n_inside", "mean_pen_mm", "max_pen_mm",
                        "n_active", "mean_ct_mm"]
        _log_path_s6 = os.path.join(_logs, "optimize_stage6.log")
        ...
        if ENABLE_STAGE6 and _has_inter and contact_cache is not None:
            _dump_dir_s6 = os.path.join(_diag, "stage6_debug")
            _dump_stage_debug_meshes(
    ...
    assert parsed_contact_map, (
        "Stage 4 produced an empty contact map — interaction segment may be empty. "
        "All input videos are expected to contain hand-object interaction."
    )
    ...
    if parsed_contact_map:
        ...
        def _compute_stage3_anatomy_gate():
            ...
            _csv_path = os.path.join(_diag, "stage3_hand_anatomy_gate.csv")
            _json_path = os.path.join(_diag, "stage3_hand_anatomy_gate.json")
            _corrections_path = os.path.join(_diag, "stage3_hand_anatomy_corrections.jsonl")
            _sampled_list = [int(x) for x in np.asarray(sampled_indices).reshape(-1).tolist()]
    ...
    save_path = str(layout.optimized_transform_sequence_json)
    ...
    with torch.no_grad():
        for i in range(0, num_total_frames, render_batch_size):
            ...
            current_obj_meshes, current_hand_meshes, current_hand_joints = current_model()
            ...
            for j in range(current_batch_size):
                ...
                os.makedirs(str(layout.optimized_object_meshes_dir), exist_ok=True)
                save_path = os.path.join(str(layout.optimized_object_meshes_dir), f"{i*render_batch_size+j:05d}.obj")
            ...
            if getattr(args, "export_ho3d_eval_data", False):
    ...
    video_path = os.path.join(_prev_v, "optimized_fitting.mp4")
    ...
    sparse_final_video_path = os.path.join(_prev_v, "optimized_fitting_SPARSE_final.mp4")
    ...
    all_hand_joints = np.concatenate(all_hand_joints_list, axis=0) if all_hand_joints_list else None

    # save hand-object mesh sequences using k3d to html
    save_k3d_visualization(
        str(layout.stage3_previews_dir),
        seq_path,
        all_obj_verts,
        all_obj_faces,
        all_hand_verts,
        all_hand_faces,
        html_path=str(layout.visualization_3d_html),
        env_depths_dir=str(layout.env_depths_dir),
    )
    ...
    # Determine output directory (always contact-based for interaction videos)
    output_dir = str(layout.optimized_hoi_contact_seq_dir)
    ...
    save_hoi_sequence(
        ...
        canonical_faces_np,
        final_obj_scale,
        final_obj_rot_mat_col_major,
        final_obj_trans,
        mano_root_full_tensor,
        mano_pose_full_tensor,
        mano_trans_full_tensor,
        is_right_full_tensor,
        all_obj_verts,
        all_obj_faces,
        all_hand_verts,
        all_hand_faces,
        mesh_root=str(layout.optimized_meshes_dir),
    )

    if getattr(args, "export_ho3d_eval_data", False):
        if all_hand_joints is None:
            raise RuntimeError("HO3D eval export requested but hand joints were not collected")
        eval_data_path = layout.stage3_final_dir / "ho3d_eval" / "eval_data.npy"
        save_magic_hoi_eval_data(
            output_path=eval_data_path,
            seq_name=os.path.basename(seq_path),
            intrinsics=intrinsics.detach().cpu().numpy(),
            hand_verts=all_hand_verts,
            hand_joints_openpose=all_hand_joints,
            hand_faces=all_hand_faces,
            obj_verts=all_obj_verts,
            obj_faces=all_obj_faces,
        )
        print(f"  -> Saved HO3D/MagicHOI eval data to {eval_data_path}")
...
def process_sequence(seq_path, data_output_path, worker_model_state, args, fps=30):
    ...
    layout = VideoLayout.from_root(output_seq_path)
    layout.ensure_stage_dirs()
    bbox_save_path = str(layout.global_bbox_json)
    amodal_masks_save_dir = str(layout.amodal_masks_dir)
    cropped_depths_save_dir = str(layout.cropped_depths_dir)
    cropped_metric_depths_save_dir = str(layout.cropped_metric_depths_dir)

    # Load raw frames once at the beginning
    raw_rgbs_np = load_raw_frames(str(layout.frames_rgb_dir), frame_type='rgb')
    raw_masks_np = load_raw_frames(str(layout.object_masks_dir), frame_type='mask')
    ...
    if hand_inputs.selected_hand_side is not None:
        ...
        lh_masks_path = str(layout.lh_masks_dir)
        ...
        rh_masks_path = str(layout.rh_masks_dir)
    ...
    comparison_video_path = str(layout.stage3_preview_videos_dir / "comparison_modal_amodal.mp4")
    combined_frames = []
    ...
    fit_and_visualize_pose(
        ...
        no_stage4_camera_offset=getattr(args, "no_stage4_camera_offset", False),
...
def main(args):
    ...
    # Multi-machine sharding by total_parts / part_idx
    if args.total_parts > 1:
        ...
        # Sort so every machine shards identically
        subfolders = sorted(subfolders)
        total_seqs = len(subfolders)

        # Compute this shard's range
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default=str(DEFAULT_INPUT_ROOT), help="Path to the parent directory containing sequence subfolders.")
    parser.add_argument("--data_output_path", type=str, default=str(DEFAULT_OUTPUT_ROOT), help="Output path.")
    ...
    parser.add_argument('--export_ho3d_eval_data', action='store_true', help='Export stage3/final/ho3d_eval/eval_data.npy for optional MagicHOI-style HO3D evaluation.')
    ...
    parser.add_argument(
        '--no_stage4_camera_offset',
        action='store_true',
        help="Disable Stage 4 camera-ray depth offset generation and application.",
    )
    parser.add_argument(
    ...
    parser.add_argument('--contact_indices_path', type=str, default=str(DEFAULT_CONTACT_INDICES), help="Optional path to contact hand-vertex indices JSON.")
    ...
    parser.add_argument(
        ...
        help="After Stage 3 vanilla PnP, write pnp_health.csv/json (dry-run metrics only). "
             "Legacy flag name; segmented SAM3D reset/bridge is not implemented in this release.",
    )
    parser.add_argument(
        '--sam3d_sparse_keyframes',
        dest='sam3d_sparse_keyframes',
        action='store_true',
        default=True,
    ...
    parser.add_argument(
        '--no_sam3d_sparse_keyframes',
        dest='sam3d_sparse_keyframes',
        action='store_false',
        help="Disable default SAM3D object pose tracking diagnostics/initialization.",
    )
    parser.add_argument(
        '--sam3d_sparse_stride',
        type=int,
