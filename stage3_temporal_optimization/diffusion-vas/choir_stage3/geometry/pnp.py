...
from choir_stage3.geometry.utils import farthest_point_sampling_torch
...
def angular_distance(rvec1, rvec2):
    """Angle difference between two rotation vectors (radians)."""
...
def run_pnp_1stage(model, video, queries_2d, queries_3d, masks, K, R, T, device, output_dir=None, vis_threshold=0.5):
    ...
    for t in range(T_frames):
        ...
        if len(valid_2d) < 8:
            ...
            # -----------------------------------------------------------
            # 1. RANSAC (inliers first; reject noisy points)
            # -----------------------------------------------------------
            success_ransac, rvec_ransac, tvec_ransac, inliers = cv2.solvePnPRansac(valid_3d,
                                                                                   valid_2d,
                                                                                   K,
            ...
            success_refine = False  # whether refinement succeeded
            ...
            if success_ransac:
                ...
                # -----------------------------------------------------------
                # 2. Dynamic planarity check (after RANSAC)
                # -----------------------------------------------------------
                # Center the inlier points
                pts3d_centered = pts3d_in - np.mean(pts3d_in, axis=0)
                u, s, vh = np.linalg.svd(pts3d_centered)

                # Guard against divide-by-zero / degenerate clouds
                if s[0] < 1e-6:
                    is_planar = True  # nearly a single point -> treat as degenerate/planar
                else:
                    is_planar = (s[2] / s[0]) < 0.05

                # -------------------------------------------------------
                # 3. Branched refinement
                # -------------------------------------------------------
                if is_planar:
                    # Strategy A: planar IPPE
                    # Strategy A: planar IPPE
                    try:
                        # 1. Accept all return values for OpenCV version differences
                        # solvePnPGeneric may return 3-tuple (count, rvecs, tvecs) or 4-tuple (+ error)
                        ippe_results = cv2.solvePnPGeneric(pts3d_in, pts2d_in, K, None, flags=cv2.SOLVEPNP_IPPE)

                        # 2. Index into returns (rvecs=2nd, tvecs=3rd)
                        # index 0: int (number of solutions)
                        # index 1: tuple of rvecs
                        # index 2: tuple of tvecs
                        rvecs_cand = ippe_results[1]
                        tvecs_cand = ippe_results[2]

                        # 3. Disambiguate: pick solution closest in angle to previous frame
                        best_idx = 0
                    ...
                    # Strategy B: general 3D SQPNP
                    # SQPNP ignores useExtrinsicGuess; it is a global solver
                    solve_flag = cv2.SOLVEPNP_SQPNP if hasattr(cv2, 'SOLVEPNP_SQPNP') else cv2.SOLVEPNP_ITERATIVE

                    # Note: useExtrinsicGuess only matters for ITERATIVE; SQPNP ignores it safely
            ...
            # -------------------------------------------------------
            # 4. Decide which result to keep
            # -------------------------------------------------------
            if success_ransac and success_refine:
                # Refinement succeeded; update history
                prev_rvec = rvec
                prev_tvec = tvec
            elif success_ransac:
                # Refinement failed (e.g. IPPE); fall back to RANSAC if available
                rvec = rvec_ransac
                tvec = tvec_ransac
                prev_rvec = rvec
                prev_tvec = tvec
            else:
                # RANSAC also failed; reuse previous frame
                rvec, tvec = prev_rvec, prev_tvec

        # -------------------------------------------------------
        # 5. Coordinate conversion (must run outside the if/else)
        # -------------------------------------------------------
        # Convert rotation vector to matrix
