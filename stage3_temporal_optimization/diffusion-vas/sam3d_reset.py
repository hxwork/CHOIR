...
_SAM3D_FREEZE_KEYS = ("shape", "scale")
...
class Sam3dResetController:
    ...
    def keyframe(self, image_np, mask_np, frame_idx: int = 0, seed: int = 42) -> dict:
        ...
        with self._sam3d_cwd(), _temporal_hook(self.inference, self.ctrl), _suppress_sam3d_logs(self.quiet):
            # The notebook Inference.__call__ defaults to with_mesh_postprocess=True,
            # which runs nvdiffrast hole filling while exporting a GLB. Diffusion-VAS
            # only consumes pose fields here, so skip mesh postprocess to avoid
            # compiling nvdiffrast_plugin during SAM3D keyframe initialization.
            out = self.inference._pipeline.run(
