...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to input occluder data.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
...
def main():
    ...
    with open(args.occluder_data_path, 'r') as file:
        occluder_data = json.load(file)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch_data in enumerate(train_dataloader):
            ...
            rgb_res = batch_data['rgb_res']
            rgb_res = [x[..., [2, 1, 0]] for x in rgb_res]  # BGR to RGB (dim -1 is channels)
            rgb_res = [x.squeeze(0) for x in rgb_res]
            rgb_pixels = torch.stack(rgb_res, dim=0).to(modal_pixels.device)
            rgb_pixels = rgb_pixels.permute(0, 3, 1, 2).float() / 127.5 - 1.0

            

            for j in range(5):

                sample = random.choice(occluder_data)
                occluder_rles = sample['segmentation']

                for i in range(25):
                    occluder_mask = maskUtils.decode(occluder_rles[i])
                    occluder_mask = torch.tensor(occluder_mask).unsqueeze(0).repeat(3, 1, 1)
                    occluder_mask = F.interpolate(occluder_mask.unsqueeze(0), size=(128, 256), mode='bilinear', align_corners=False)


                    occluder_mask = occluder_mask * 2 - 1

                    if i==0:
                        plt.imshow((occluder_mask[ 0, :, :, :].permute(1, 2, 0).cpu().numpy() + 1) / 2)
                        plt.show()


                    occluder_mask = occluder_mask.to(modal_pixels.device)
                    modal_pixels[:, i, :, :, :] = torch.where(occluder_mask == 1, torch.tensor(-1), modal_pixels[:, i, :, :, :])

                modal_binary = (modal_pixels == 1).float()
                amodal_binary = (amodal_pixels == 1).float()
                tmp_intersection = torch.sum(modal_binary * amodal_binary, dim=(2, 3, 4))
                tmp_union = torch.sum(modal_binary + amodal_binary, dim=(2, 3, 4)) - tmp_intersection
                tmp_iou = (tmp_intersection / tmp_union).mean()


                if tmp_iou < 0.7:
                    break

            modal_mask = (modal_pixels > 0).float()
            amodal_mask = (amodal_pixels > 0).float()
            modal_background = 1 - modal_mask
            amodal_background = 1 - amodal_mask

            modal_rgb_pixels = rgb_pixels * modal_mask + modal_background
            amodal_rgb_pixels = rgb_pixels * amodal_mask + amodal_background

            # ⚠️ Variable names are intentionally swapped to match amodal segmentation training code ⚠️
            # ⚠️ Therefore, Variable names no longer reflect original meanings 😛 ⚠️
            modal_pixels, rgb_pixels, amodal_pixels = modal_rgb_pixels, amodal_pixels, amodal_rgb_pixels
            ...
            with accelerator.accumulate(unet):
                ...
                conditional_rgb_pixel_values = \
                    torch.randn_like(conditional_rgb_pixel_values) * cond_sigmas + conditional_rgb_pixel_values
                # conditional_rgb_latents = tensor_to_vae_latent(conditional_pixel_values, vae)[:, 0, :, :, :]
                conditional_rgb_latents = tensor_to_vae_latent(conditional_rgb_pixel_values, vae)
                conditional_rgb_latents = conditional_rgb_latents / vae.config.scaling_factor

                conditional_latents = torch.cat([conditional_latents, conditional_rgb_latents], dim=2)
                ...
                if args.conditioning_dropout_prob is not None:
                    ...
                    # conditional_rgb_latents = image_mask * conditional_rgb_latents
                    ...
                    if (
                        ...
                        with torch.autocast(
                            ...
                            for step, batch_data in enumerate(val_dataloader):
                                ...
                                # print("rgb_pixels:", rgb_pixels.shape, rgb_pixels.min(), rgb_pixels.max())
                                ...
                                for j in range(5):
                                    ...
                                    for i in range(25):
                                        occluder_mask = maskUtils.decode(occluder_rles[i])
                                        occluder_mask = torch.tensor(occluder_mask).unsqueeze(0).repeat(3, 1, 1)
                                        occluder_mask = F.interpolate(occluder_mask.unsqueeze(0), size=(128, 256),
                                                                      mode='bilinear', align_corners=False)

                                        occluder_mask = occluder_mask * 2 - 1
                                        occluder_mask = occluder_mask.to(modal_pixels.device)
                                        modal_pixels[:, i, :, :, :] = torch.where(occluder_mask == 1, torch.tensor(-1),
                                                                                  modal_pixels[:, i, :, :, :])

                                    modal_binary = (modal_pixels == 1).float()
                                    amodal_binary = (amodal_pixels == 1).float()
                                    tmp_intersection = torch.sum(modal_binary * amodal_binary, dim=(2, 3, 4))
                                    tmp_union = torch.sum(modal_binary + amodal_binary,
                                                          dim=(2, 3, 4)) - tmp_intersection
                                    tmp_iou = (tmp_intersection / tmp_union).mean()
