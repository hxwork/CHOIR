...
class CoTrackerPredictor(torch.nn.Module):
    ...
    def _compute_dense_tracks(self, video, grid_query_frame, grid_size=80, backward_tracking=False):
        *_, H, W = video.shape
        ...
        for offset in range(grid_step * grid_step):
            ...
            ox = offset % grid_step
            oy = offset // grid_step
            grid_pts[:, :, 1] = (torch.arange(grid_width).repeat(grid_height) * grid_step + ox)
            grid_pts[:, :, 2] = (torch.arange(grid_height).repeat_interleave(grid_width) * grid_step + oy)
    ...
    def _compute_sparse_tracks(
        ...
        if queries is not None:
            ...
            if segm_mask is not None:
                # in case of a segmentation mask, we sample points from it
                segm_mask = F.interpolate(segm_mask, tuple(self.interp_shape), mode="nearest")

                # Sample points directly from the mask using Farthest Point Sampling
                non_zero_coords = (segm_mask[0, 0] > 0).nonzero(as_tuple=False)

                if non_zero_coords.shape[0] > 0:
                    num_queries = grid_size * grid_size
                    # nonzero returns (row, col) which is (y, x). Flip to get (x, y).
                    coords_xy = torch.flip(non_zero_coords, dims=[1]).float()

                    if coords_xy.shape[0] > num_queries:
                        sampled_pts = farthest_point_sampling_torch(coords_xy, num_queries)
                        grid_pts = sampled_pts.unsqueeze(0)
                    else:
                        # Not enough points, take all of them
                        grid_pts = coords_xy.unsqueeze(0)
                else:
                    # No points found in mask
                    grid_pts = torch.zeros(B, 0, 2, device=video.device)
            else:
                # No mask provided, sample a regular grid
        ...
        tracks *= tracks.new_tensor([(W - 1) / (self.interp_shape[1] - 1), (H - 1) / (self.interp_shape[0] - 1)])
        return tracks, visibilities
    ...
    def _compute_backward_tracks(self, video, queries, tracks, visibilities):
        ...
        inv_tracks, inv_visibilities, *_ = self.model(video=inv_video, queries=inv_queries, iters=6)
    ...
    def forward(
        ...
        if is_first_step:
            ...
            if queries is not None:
                ...
                if add_support_grid:
                    grid_pts = get_points_on_a_grid(self.support_grid_size, self.interp_shape, device=video_chunk.device)
                    grid_pts = torch.cat([torch.zeros_like(grid_pts[:, :, :1]), grid_pts], dim=2)
                    queries = torch.cat([queries, grid_pts], dim=1)
            elif grid_size > 0:
                grid_pts = get_points_on_a_grid(grid_size, self.interp_shape, device=video_chunk.device)
                self.N = grid_size**2
                queries = torch.cat(
                    [torch.ones_like(grid_pts[:, :, :1]) * grid_query_frame, grid_pts],
                    dim=2,
        ...
        video_chunk = F.interpolate(video_chunk, tuple(self.interp_shape), mode="bilinear", align_corners=True)
        ...
        if self.v2:
            ...
            tracks, visibilities, confidence, __ = self.model(video=video_chunk, queries=self.queries, iters=6, is_online=True)
        if add_support_grid:
