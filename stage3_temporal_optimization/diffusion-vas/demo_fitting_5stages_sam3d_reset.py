...
def fit_and_visualize_pose(
    ...
    def _run_stage5():
        ...
        if _has_inter:
            ...
            _dump_stage_debug_meshes(
                ...
                contact_cache, i0, i1, sampled_indices)
        if _has_inter and contact_cache is not None:
            contactmap_cache = contact_cache
    ...
    if contactmap_cache is not None and _has_inter:
        _inter_full_indices = np.asarray(sampled_indices[i0:i1], dtype=np.int64)

        def _save_contactmap_from_cache(confidence_attr, sequence_suffix, project_all_hand_vertices=False):
            hand_conf_inter = contact_cache_to_dense_hand_confidence(
                contactmap_cache,
                num_hand_verts=all_hand_verts.shape[1],
                confidence_attr=confidence_attr,
            )
            _hand_verts_inter = all_hand_verts[_inter_full_indices] if project_all_hand_vertices else None
            _obj_verts_inter = all_obj_verts[_inter_full_indices] if project_all_hand_vertices else None
            obj_conf_inter = contact_cache_to_object_vertex_confidence(
                contactmap_cache,
                all_obj_faces,
                num_obj_verts=all_obj_verts.shape[1],
                confidence_attr=confidence_attr,
                hand_verts=_hand_verts_inter,
                obj_verts=_obj_verts_inter,
            )
            hand_conf_sampled = np.zeros((num_sampled_frames, all_hand_verts.shape[1]), dtype=np.float32)
            obj_conf_sampled = np.zeros((num_sampled_frames, all_obj_verts.shape[1]), dtype=np.float32)
            hand_conf_sampled[i0:i1] = hand_conf_inter
            obj_conf_sampled[i0:i1] = obj_conf_inter
            hand_conf_full = interpolate_vertex_confidence(sampled_indices, hand_conf_sampled, num_total_frames)
            obj_conf_full = interpolate_vertex_confidence(sampled_indices, obj_conf_sampled, num_total_frames)
            save_contactmap_sequence(
                os.path.basename(seq_path),
                all_obj_verts,
                all_obj_faces,
        ...
        _save_contactmap_from_cache("contact_confidence", "contactmap")
        _save_contactmap_from_cache("observed_contact_confidence", "contactmap_perframe")
        _save_contactmap_from_cache(
            "raw_observed_contact_confidence",
            "contactmap_raw",
            project_all_hand_vertices=True,
        )
        _save_contactmap_from_cache(
            "raw_geometry_contact_confidence",
            "contactmap_raw_geometry",
            project_all_hand_vertices=True,
        )

    distance_hand_conf, distance_obj_conf = compute_distance_contact_confidence(
        all_hand_verts,
        all_obj_verts,
        distance_scale=0.02,
    )
    save_contactmap_sequence(
        ...
        distance_hand_conf,
        distance_obj_conf,
        sequence_suffix="contactmap_distance",
    )
