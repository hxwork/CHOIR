...
def resolve_sam3d_ref_obj_mask_path(seq_path: str, clip_frame_idx: int) -> Optional[str]:
    ...
    For hold videos the sequence root may also contain a ``0.png`` bootstrap
    image/mask from SAM3D assets. Reference frames must instead use the per-frame
    masks under ``obj_masks`` so a non-zero starting frame gets the matching mask.
    """
    mask_dir = os.path.join(seq_path, "obj_masks")
...
def load_sam3d_ref_obj_mask(seq_path: str, clip_frame_idx: int) -> tuple[Optional[np.ndarray], Optional[str]]:
    """Load a binary object mask for a SAM3D reference frame from ``obj_masks``."""
