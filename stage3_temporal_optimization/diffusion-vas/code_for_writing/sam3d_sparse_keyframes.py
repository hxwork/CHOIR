...
def run_sam3d_rotation_dense_from_mask_onset(
    *,
    ...
    if warmup_indices_full:
        print(
            f"[SAM3D-ROT] ref keyframe @0; warmup sampled {warmup_indices_full[0]}..{warmup_indices_full[-1]} "
            f"→ {len(warmup_plan)} follow(s); dense output {dense_indices[0]}..{dense_indices[-1]} "
            f"→ {len(dense_plan)} follow(s)"
        )
    else:
        print(
            f"[SAM3D-ROT] ref keyframe @0; dense output {dense_indices[0]}..{dense_indices[-1]} "
            f"→ {len(dense_plan)} follow(s)"
        )
    if _saved > 0:
    ...
    # --- Reference keyframe @ sampled 0 (matches demo_proj_temporal reference_keyframe) ---
    raw0 = int(sampled_indices[0])
    img0, msk0 = _prepare_frame(raw0)
    if img0 is None:
        print(f"[SAM3D-ROT] skip: cannot load or mask too small at ref sampled_slot=0 clip_idx={raw0}")
        return None, empty_poses
    _log_line("ref_keyframe", 0, raw0, "keyframe")
    try:
        out0 = ctrl.keyframe(img0, msk0, frame_idx=0, seed=int(seed))
        frozen_pose = {
        ...
        print(f"[SAM3D-ROT] reference keyframe @0 failed: {_e}")
        traceback.print_exc()
        return None, empty_poses
    ...
    def _run_follow(sidx: int, phase: str, record_dense_outputs: bool) -> bool:
        ...
        try:
            ...
            try:
                out = ctrl.follow_run(img, msk_u8, seed=int(seed))
            except Exception:
                ctrl.follow_abort()
                raise
            dec = {
            ...
            if outlier_filter:
                ...
                if residual_deg > float(outlier_max_angle_deg):
                    is_local_out = True
            commit = not is_local_out
            ctrl.follow_end(int(sidx), commit=commit, decoded=dec)
        except Exception as _e:
    ...
    summary: Dict[str, Any] = {
        ...
        "reference_keyframe_sampled_idx": 0,
