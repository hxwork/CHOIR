...
def fit_and_visualize_pose(
    ...
    if os.path.exists(checkpoint_path):
        ...
        if sam3d_sparse_keyframes:
            ...
            for _fn in (
                ...
                try:
                    with open(_jp) as _jf:
                        _gate_mode = str(json.load(_jf).get("suggested_mode", "translation_likely"))
                    if _gate_mode not in ("translation_likely", "rotation_likely"):
                        _gate_mode = "translation_likely"
                    print(f"[SAM3D-GATE] checkpoint resume: suggested_mode={_gate_mode} (from {_jp})")
                    break
                except Exception:
                    continue
            ...
            if _gate_mode == "translation_likely":
                ...
                print("[SAM3D-GATE] checkpoint resume: rotation_likely dense SAM3D + PnP compare runs after Stage 2 — skipped here.")
            else:
    ...
    if not _ckpt_loaded:
        ...
        # Post–Stage 1 motion profile ``suggested_mode`` gates SAM3D sparse (translation) vs dense (rotation).
        sam3d_gate_suggested_mode = "translation_likely"
        ...
        def _run_stage3():
            ...
            def _emit_interaction_policy_diagnostics():
                ...
                if prof is None:
                    return
                raw_mode = str(prof.get("suggested_mode", "translation_likely"))
                ...
                lines = [
                    f"policy_from_profile={policy}  (suggested_mode={raw_mode})",
            ...
            stage3_object_pose_init[approaching_end_idx:] = pnp_poses[(approaching_end_idx - pnp_start_frame_idx):]
        ...
        try:
            ...
            if (approaching_end_idx is not None and interaction_end_idx is not None
                ...
                sam3d_gate_suggested_mode = str(_prof_gate.get("suggested_mode", "translation_likely"))
                ...
                print(f"\n[SAM3D-GATE] post–Stage 1 → {_gate_json}  suggested_mode={sam3d_gate_suggested_mode}")
            else:
                print("\n[SAM3D-GATE] skip profile (invalid 2–3 interaction range); suggested_mode stays translation_likely.")
        ...
        if sam3d_sparse_keyframes and sam3d_gate_suggested_mode == "rotation_likely":
            ...
            if _onset >= num_sampled_frames:
                ...
                _rot_summary, _pose_snaps = run_sam3d_rotation_dense_from_mask_onset(
                    ...
                    outlier_max_angle_deg=float(sam3d_rot_outlier_max_angle_deg),
                ...
                try:
                    ...
                    queries_2d, queries_3d = generate_queries(
                        final_posed_mesh,
                        mesh_canonical_scaled,
                        sampled_pred_amodal_masks_np[_onset],
                        focal_length[_onset : _onset + 1],
                        principal_point[_onset : _onset + 1],
                        grid_size=15,
                        device=device,
                    )
    ...
    def _run_stage5():
        ...
        CONTACT_OBJ_GRAD_SCALE_S5 = (0.0, 0.0, 0.0)
        GAP_OBJ_GRAD_SCALE_S5 = (0.0, 0.0, 0.0)
        PATCH_OBJ_GRAD_SCALE_S5 = (0.0, 0.0, 0.0)
        TEMPLATE_OBJ_GRAD_SCALE_S5 = (1.0, 1.0, 1.0)
        NEIGHBOR_TEMPLATE_BRIDGE_OBJ_GRAD_SCALE_S5 = (0.05, 0.05, 0.6)
        # Keep the global hand trajectory close to Stage 3 / 2D evidence.
        # Contact losses are routed away from mano_trans below, so hand_2d can
        # stay as a strong global-position prior while fingers still refine.
        ...
        for step in loop_s5:
            ...
            if _has_inter and contact_cache is not None:
                ...
                _amano_contact = run_amano(
                    multi_frame_model.hand_model_amano,
                    multi_frame_model.mano_trans[i0:i1].detach()[None],
                ...
                if frozen_template_conf is None:
                    ...
                    # only to close the hand-object gap.  The hand wrist/root are
                    # not detached here, so the bootstrap can move the global hand
                    # toward the object while 2D and wrist anchors limit drift.
                    bootstrap_gap_raw_s5, gap_info_s5 = compute_confident_gap_closing_loss(
                        _hv_i, _ov_i.detach(), faces, contact_cache,
            ...
            with torch.no_grad():
                ...
                if _n_template > 0 and _template_state != "no_template_bootstrap_gap":
                    ...
                    if frozen_template_conf is None:
                        ...
                        elif _template_state == "bootstrap":
                            bootstrap_template_seen = True
                            _prop_conf, _prop_face, _prop_bary, _reliable_frames = _propagate_template(
                                _conf_full, _anchor_face_full, _anchor_bary_full, _obs_conf_full,
                                allow_bootstrap=True)
                            _log_reclassify("  [reclassify] grasp template bootstrap is provisional; not frozen")
