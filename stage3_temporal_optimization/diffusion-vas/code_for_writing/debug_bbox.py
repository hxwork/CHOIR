...
def load_hand_data(seq_path, num_frames):
    ...
    data = {key: json.load(open(p)) if os.path.exists(p) else {} for key, p in paths.items()}

    all_frames_data = []
    for i in range(num_frames):
        k = str(i)
        frame_hands = {}
        if k in data['lh_bbox']:
            frame_hands['lh'] = {'bbox': data['lh_bbox'].get(k)}
            if k in data['lh_kpts']:
                frame_hands['lh']['kpts'] = data['lh_kpts'].get(k)
        if k in data['rh_bbox']:
