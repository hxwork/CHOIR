...
def fit_and_visualize_pose(
    ...
    metric_stage3_raw_export = ablation == "stage3_raw_metric"
    metric_post_gfm_export = ablation == "post_gfm_metric"
    metric_cutpoint_export = metric_stage3_raw_export or metric_post_gfm_export
    ...
    if os.path.exists(checkpoint_path) and checkpoint_is_usable:
        try:
            ...
            _ckpt_ray_scale_applied = _ckpt_probe.get("ray_scale_applied", None)
            if metric_stage3_raw_export and _ckpt_ray_scale_applied is False:
                checkpoint_is_usable = False
                print(
                    "  stage3_raw_metric now exports the post-ray-scale Stage 3 state. "
                    "Existing checkpoint was saved before ray-scale alignment; "
                    "ignoring it for this run."
                )
        ...
        torch.save(
            {
                ...
                'ablation': ablation,
                'ray_scale_applied': True,
                'metric_cutpoint_export': bool(metric_cutpoint_export),
    ...
    def _run_stage4():
        ...
        if metric_post_gfm_export:
            print("[Ablation] post_gfm_metric: GFM depth correction complete; skipping contact-map estimation.")
            return
    ...
    if metric_stage3_raw_export:
        print("[Ablation] stage3_raw_metric: skipping Stage 4/5/6 and exporting Stage 3 raw result.")
    elif disable_stage4:
    ...
    if metric_post_gfm_export:
        print("[Ablation] post_gfm_metric: skipping Stage 5/6 and exporting post-GFM result.")

    if (not metric_cutpoint_export) and not parsed_contact_map:
        print(
    ...
    if (not metric_cutpoint_export) and parsed_contact_map:
        # ================================================================
