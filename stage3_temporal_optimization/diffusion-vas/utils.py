...
def init_relative_translation(
        init_pose_T,  # (3,) 初始帧的 GT Pose
        masks,  # (T, H, W) Amodal Mask 序列
        fx,
        fy,  # (2,) 内参
        init_frame_idx=0  # 初始化帧的索引，默认为第0帧
):
    """
    基于相对运动初始化平移向量
    
    Args:
        init_pose_T: 初始帧的 GT Pose (3,)
        masks: Amodal Mask 序列 (T, H, W)
        fx, fy: 相机内参
        init_frame_idx: 初始化帧的索引，默认为0
                       - 如果为0，则从第0帧开始向后计算
                       - 如果非0，则从该帧开始前向、后向分别计算
    
    Returns:
        initial_translations: (T, 3) 从第0帧到最后一帧的平移向量序列
    """
    ...
    def compute_translation_from_reference(ref_T, ref_u, ref_v, ref_Z, target_mask):
        ...
        # 将 2D 位移映射为 3D 位移 (Back-projection)
